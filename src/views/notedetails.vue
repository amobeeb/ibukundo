<template>
 <div id="app">
  ...
  >
    <v-list-item>
      <v-list-item-content>
        <v-list-item-title>{{note_id}}</v-list-item-title>
      </v-list-item-content>
    </v-list-item>
    ...
    <v-card-actions>
      <v-btn @click="back()" large color="primary">Back</v-btn>
       <v-btn @click="delnote()" color="error">Delete</v-btn>
    </v-card-actions>
  </v-card>
...
export default {
    ...
    beforeRouteEnter(to,from,next){
      ...
      db.collection('users').doc(user.uid).collection('notes').where('note_id', '==', to.params.note_id).get()
      .then(querySnapshot=>{
    ...
    methods:{
        fetchdata(){
            var user = firebase.auth().currentUser;
             db.collection('users').doc(user.uid).collection('notes').where('note_id','==', this.$route.params.note_id).get()
             .then(querySnapshot=>{
                 querySnapshot.forEach(doc=>{
